...
const SettingItem = ({ icon, label, value, onClick, last = false }: Props) => (
  <button onClick={onClick} className="flex items-center w-full px-2 py-4 bg-transparent">
    ...
    {!last && <div className="absolute left-12 right-2 bottom-0 h-px bg-[var(--tg-hint-color)] opacity-30"></div>}
  </button>
)
