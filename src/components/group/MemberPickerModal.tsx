...
// Модалка выбора участника группы (для "Paid by") — стилистически как GroupPickerModal.
  ...
  if (avatarUrl) {
    return (
      <img
        src={avatarUrl}
        alt={name}
        className="mr-3 rounded-xl object-cover"
        style={{ width: 34, height: 34 }}
      />
    );
  }
  // цветной фон (простой хэш по букве)
...
export default function MemberPickerModal({
  ...
  // загрузка всех участников пачками
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // крутим страницы, пока не закончатся
          ...
          {list.map((m, idx) => {
            ...
            return (
              <div key={m.user.id} className="relative">
                <button
                  type="button"
                  onClick={() => { onSelect({ id: m.user.id, name, avatar_url: (m.user as any)?.avatar_url }); if (closeOnSelect) onClose(); }}
