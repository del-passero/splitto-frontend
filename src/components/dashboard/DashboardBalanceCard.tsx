...
// Контент карточки «Мой баланс» (без собственной обёртки CardSection)
...
export default function DashboardBalanceCard() {
  ...
  // Полный набор валют: сперва по последнему использованию, затем недостающие
  ...
  // Абсолютные значения по сторонам
  ...
  // Активные чипы: из UI или 2 последние (с fallback на ненулевые)
  const active = useMemo(() => {
    ...
    const defHasValues = def.some((c) => {
      const v = absBySide.get(c)
      return v && (v.left > 0 || v.right > 0)
    })
    if (defHasValues) return def
  ...
  // Списки строк (каждая валюта — новая строка)
  ...
  return (
    <div
      ...
      "
    >
      {/* Чипы валют (без 3D, скругление меньше) */}
      ...
      {/* Две половины: слева «Я должен», справа «Мне должны». Каждая валюта — с новой строки */}
